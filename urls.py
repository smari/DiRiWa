...
urlpatterns = patterns('',
	...
	(r"^regions/add/$",						CreateView.as_view(model=Region, success_url="/regions/region/%(id)d/", template_name_suffix="_new")),
	(r"^regions/edit/(?P<pk>\d+)/$",				UpdateView.as_view(model=Region, success_url="/regions/region/%(id)d/", template_name_suffix="_edit")),
	(r"^regions/region/(?P<pk>\d+)/$",			RegionView.as_view()),
	(r"^countries/$",							ListView.as_view(model=Country, context_object_name="countries")),
	(r"^countries/add/$",						CreateView.as_view(model=Country, success_url="/countries/country/%(id)d/", template_name_suffix="_new")),
	(r"^countries/edit/(?P<pk>\d+)/$",			UpdateView.as_view(model=Country, success_url="/countries/country/%(id)d/", template_name_suffix="_edit")),
	(r"^countries/country/(?P<pk>\d+)/$",		CountryView.as_view()),
	(r"^topics/$",							ListView.as_view(model=Topic, context_object_name="topics")),
	(r"^topics/add/$",							CreateView.as_view(model=Topic, success_url="/topics/topic/%(id)d/", template_name_suffix="_new")),
	(r"^topics/edit/(?P<pk>\d+)/$",				UpdateView.as_view(model=Topic, success_url="/topics/topic/%(id)d/", template_name_suffix="_edit")),
