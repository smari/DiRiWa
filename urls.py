...
urlpatterns = patterns('',
	...
	(r"^regions/region/(?P<pk>\d+)/$",	RegionDetailView.as_view()),
	...
	(r"^countries/country/(?P<pk>\d+)/$",	CountryDetailView.as_view()),
	...
	(r"^topics/topic/(?P<pk>\d+)/$",	TopicDetailView.as_view()),
	(r"^sections/$",			ListView.as_view(model=EntityTopic, context_object_name="sections")),
	(r"^sections/add/$",			login_required(SectionCreateView.as_view())),
	...
	(r"^sections/section/(?P<pk>\d+)/$",	SectionDetailView.as_view()),
